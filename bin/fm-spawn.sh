...
if [ "$HARNESS" = claude ] && [ "$RAW_LAUNCH" -eq 1 ]; then
  ...
  CLAUDE_PROBE_INNER="( FM_RAW_CLAUDE_PROBE=1; $CLAUDE_PROBE_PATH_SETUP fm_raw_claude_resolved=\$(command -v -- $(shell_quote "$RAW_HARNESS_EXECUTABLE") 2>/dev/null || true); fm_raw_claude_version=; fm_raw_claude_exit_code=127; if [ -n \"\$fm_raw_claude_resolved\" ]; then case \"\$fm_raw_claude_resolved\" in /*) ;; */*) fm_raw_claude_base=\${fm_raw_claude_resolved##*/}; fm_raw_claude_dir=\${fm_raw_claude_resolved%/*}; fm_raw_claude_dir=\$(cd \"\${fm_raw_claude_dir:-.}\" 2>/dev/null && pwd -P) && fm_raw_claude_resolved=\"\$fm_raw_claude_dir/\$fm_raw_claude_base\" || fm_raw_claude_resolved= ;; *) fm_raw_claude_resolved=\"\$(pwd -P)/\$fm_raw_claude_resolved\" ;; esac; fi; if [ -n \"\$fm_raw_claude_resolved\" ]; then fm_raw_claude_version=\$(\"\$fm_raw_claude_resolved\" --version 2>&1); fm_raw_claude_exit_code=\$?; fi; printf '%s' \"\$fm_raw_claude_resolved\" > $(shell_quote "$CLAUDE_PROBE_PATH"); printf '%s' \"\$fm_raw_claude_version\" > $(shell_quote "$CLAUDE_PROBE_VERSION"); printf '%s' \"\$fm_raw_claude_exit_code\" > $(shell_quote "$CLAUDE_PROBE_STATUS"); : > $(shell_quote "$CLAUDE_PROBE_READY") )"
  ...
  "$FM_ROOT/bin/fm-claude-rc-off.sh" check-default-version "$CLAUDE_PANE_EXECUTABLE" "$CLAUDE_PANE_VERSION" >/dev/null || {
    echo "error: raw Claude launch-pane preflight failed for '$CLAUDE_PANE_EXECUTABLE' (${CLAUDE_PANE_VERSION:-version unavailable}); refusing launch" >&2
    exit 1
  }
fi
